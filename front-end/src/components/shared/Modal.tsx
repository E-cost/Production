...
const SharedModal: React.FC<ModalProps> = ({ id, title, cartItems, quantities, t, offcanvasCartRef }) => {
    ...
    const onSubmit = async (e: any) => {
        ...
        try {
            const response = await fetch(`${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/orders/create`, {
                method: 'POST',
                headers: {
                ...
                <div className={`modal-body ${styles.modal_body}`}>
                    <div className={styles.section_one}>
                        ...
                        <div className={styles.total_cost}>
                            ...
                            <p>{cartItems.reduce((total, item, index) => {
                                    const itemTotal = item.price_byn * quantities[index];
                                    return total + (itemTotal * 1.2);
                                }, 0).toFixed(2)} BYN
                            </p>
                        </div>
