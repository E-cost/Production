...
export default function ItemPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(`${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/items/${category}/${id}`);
    ...
    useEffect(() => {
        const fetchImageUrls = async () => {
            ...
            if (!storedImageUrls) {
                try {
                    const response = await fetch(`${process.env.REACT_APP_API_HOST}:${process.env.REACT_APP_API_PORT}/photos/${id}`);
                    if (!response.ok) {
