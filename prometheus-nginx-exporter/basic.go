package prometheus_nginx_exporter
...
func ScanBasicsStats(r io.Reader) ([]NginxStats, error) {
	...
	for s.Scan() {
		...
		if fields[0] == "Reading:" {
			...
			watingConns := Connections{Type: "waiting", Total: 100}

			conns = append(conns, readingConns, writingConns, watingConns)
